...
// This program withdraws $10000 from accounts to simulate some type of fee being paid
...
void* th_run(void * args)
{
  ...
  barrier(0);
	...
	while(ExperimentInProgress) {
		...
		for (int j=0; j< 10; j++) {
				acc[j] = rand_r_32(&seed) % ACCOUT_NUM;
		}

		tx_count++;
		TM_BEGIN
			for (int j=0; j< 10; j++) {
				TM_WRITE(accounts[acc[j]], (TM_READ(accounts[acc[j]]) + 10000));
			}
		TM_END
	}
